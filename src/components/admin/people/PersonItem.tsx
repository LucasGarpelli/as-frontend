...
export const PersonItem = ({ item, refreshAction, onEdit }: Props) => {
    ...
    const handleDeleteButton = async () => {
        if ('Tem certeza que deseja excluir essa pessoa?') {
...
export const PersonItemPlaceholder = () => {
    return (
        <div className='w-full h-16 border border-gray-700 rounded mb-3 bg-gradient-to-r  from-gray-900 to-gray-900 animate-pulse'></div>
    )
}

export const PersonItemNotFound = () => {
    return (
        <div className='text-center py-4 text-gray-500 '>Não há Pessoas nesse grupo.</div>
    )
}
